...
- *No captured highlight — this note predates highlight capture.*
  - *The declared state and the observed state are different objects, and the system is defined by how it closes the gap.*
  - *Last-write-wins is popular because it is cheap, not because it is right.*
  - *Systems that treat divergence as an error rather than an input tend to fight their own users.*
